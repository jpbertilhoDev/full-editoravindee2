...
export default function SettingsPage() {
      ...
      try {
        ...
        if (userDoc.exists()) {
          ...
          if (userData.settings) {
            // Resetar os formulários com os dados do Firestore
            ...
            if (userData.settings.appearance) {
              ...
              if (userData.settings.appearance.theme === 'dark') {
                document.documentElement.classList.add('dark');
              } else if (userData.settings.appearance.theme === 'light') {
                document.documentElement.classList.remove('dark');
              }
            }
          } else {
            // Se não existem configurações, inicializar com os valores padrão
            await updateDoc(userDocRef, {
              settings: {
                notifications: notificationsForm.getValues(),
  ...
  const onNotificationsSubmit = async (data: NotificationsFormValues) => {
    ...
    try {
      ...
      if (userDoc.exists()) {
        ...
        // Criar documento com as configurações iniciais
        await setDoc(userDocRef, {
          uid: user.uid,
          email: user.email,
          displayName: user.displayName,
  ...
  const onAppearanceSubmit = async (data: AppearanceFormValues) => {
    ...
    try {
      ...
      // Aplicar configurações ao UI quando possível
      if (data.theme === 'dark') {
        document.documentElement.classList.add('dark');
