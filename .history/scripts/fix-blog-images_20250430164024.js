  ...
  try {
    ...
    for (const postDoc of querySnapshot.docs) {
      ...
      if (postData.imageUrl && BROKEN_URLS.includes(postData.imageUrl)) {
        console.log(
          `Corrigindo imagem quebrada no post: ${postData.title || postDoc.id}`
        );
      ...
      if (!postData.imageUrl) {
        console.log(
          `Post sem imagem encontrado: ${postData.title || postDoc.id}`
        );
        ...
        try {
          ...
          console.error(
            `Erro ao adicionar imagem ao post ${postDoc.id}:`,
            error
          );
        }
      }
