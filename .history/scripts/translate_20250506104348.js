...
if (!DEEPL_API_KEY) {
  console.error(
    "❌ DEEPL_API_KEY não encontrada. Por favor, configure-a em .env.local"
  );
  process.exit(1);
}
...
async function main() {
  try {
    ...
    if (!fs.existsSync(sourceLangDir)) {
      console.error(
        `❌ Diretório do idioma fonte ${sourceLangDir} não encontrado.`
      );
      process.exit(1);
    }
    ...
    console.log(
      `🌍 Iniciando tradução de ${sourceFiles.length} arquivos para ${TARGET_LANGUAGES.length} idiomas...`
    );
