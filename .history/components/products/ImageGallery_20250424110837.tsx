...
export default function ImageGallery({ images: propImages, title }: ImageGalleryProps) {
  ...
  return (
    <div className="relative flex flex-col md:flex-row gap-3">
      ...
      <div className="order-2 md:order-1 md:w-16 flex md:flex-col gap-2 overflow-x-auto md:overflow-y-auto pb-1 md:pb-0 md:pr-0">
        {images.map((img, index) => (
          <div
            key={index}
            className={cn(
              "relative min-w-[60px] w-[60px] h-[60px] rounded cursor-pointer transition-all duration-150",
              mainImage === index 
                ? "ring-2 ring-[#e77600] ring-offset-1" 
      ...
      <div className="order-1 md:order-2 flex-1">
        ...
        <p className="text-xs text-center text-gray-500 mt-2">Passe o mouse sobre as imagens para ampliar</p>
      </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-4">
          <div className="relative w-full max-w-4xl mx-auto">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
              <div className="flex items-center justify-center gap-3 bg-black/50 rounded-full px-4 py-2.5">
                {images.map((_, index) => (
                  <button
                    key={index}
                    className={cn(
