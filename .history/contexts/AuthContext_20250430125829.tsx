...
export const AuthProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const refreshUserData = async () => {
    try {
      ...
      // Não propagar o erro para evitar quebra da UI
    }
  };

  // Adicionar verificação periódica do token para evitar problemas de autenticação
  useEffect(() => {
    if (!user) return;
    ...
    const checkTokenInterval = setInterval(async () => {
      try {
        ...
        if (currentUser) {
          ...
          if (!token) {
            console.log("Token não disponível, usuário pode estar desconectado");
            await refreshUserData();
          }
        }
      } catch (error) {
        console.error("Erro ao verificar token:", error);
        // Em caso de erro de token, faz logout para evitar problemas de autenticação
        if (error instanceof Error && 
            (error.message.includes("auth/id-token-expired") || 
             error.message.includes("auth/user-token-expired"))) {
          console.log("Token expirado, fazendo logout do usuário");
          signOut().catch(e => console.error("Erro ao fazer logout automático:", e));
        }
      }
