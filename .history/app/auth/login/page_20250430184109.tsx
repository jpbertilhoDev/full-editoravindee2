...
export default function LoginPage() {
  ...
  // Prefetch potential redirect destinations
  useEffect(() => {
  ...
  const checkUserAndRedirect = useCallback(async () => {
    ...
    try {
      // Verificar se o usuário é um administrador - fazemos isso apenas uma vez
      ...
      // Em caso de erro, redireciona para o redirectUrl padrão
      router.push(redirectUrl);
    }
  }, [user, loading, router, redirectUrl, isRedirecting]);

  useEffect(() => {
    if (user && !loading && !isRedirecting) {
      checkUserAndRedirect();
    }
  }, [user, loading, checkUserAndRedirect, isRedirecting]);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Redirect happens via useEffect after auth state changes
    } catch (error) {
  ...
  // If checking authentication status or redirecting, show loading
  if ((loading && !isSubmitting) || isRedirecting) {
    return (
  ...
  // If already logged in, trigger redirect immediately
  if (user) {
    // Inicia o processo de redirecionamento imediatamente se ainda não foi iniciado
    if (!isRedirecting) {
      checkUserAndRedirect();
    }
    return null;
  ...
  // Render the login form only if not logged in and not redirecting
  return (
