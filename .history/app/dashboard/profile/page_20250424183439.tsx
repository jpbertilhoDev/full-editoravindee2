...
export default function ProfilePage() {
  ...
  // Monitorar mudanças no usuário para atualizar a exibição da foto
  useEffect(() => {
    if (user) {
      // Atualizar a exibição quando o usuário mudar
      setAvatarFile(null); // Limpar o arquivo local para usar a foto do perfil
    }
  }, [user?.photoURL]); // Observar especificamente mudanças na URL da foto
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Mostrar imagem comprimida imediatamente no UI enquanto faz upload
      setAvatarFile(compressedImage);
      
      // Criar novo nome de arquivo único
      ...
      // Log para depuração
      console.log(`Iniciando upload de ${blob.size} bytes com o nome ${filename}`);
      
      // Upload do blob para o Firebase Storage com metadados corretos
      ...
      // Obter URL de download da imagem com forceRefresh para evitar problemas de cache
      const downloadURL = await getDownloadURL(uploadTask.ref);
      
      console.log('URL da imagem:', downloadURL);
      
      // Salvar URL do avatar no Firestore primeiro
      const userDocRef = doc(db, "users", user.uid);
      await updateDoc(userDocRef, {
      ...
      // Atualizar o perfil do usuário no Auth (isso pode demorar para refletir)
      await updateProfile(auth.currentUser!, {
        photoURL: downloadURL
      });
      ...
      // Importante: Forçar atualização dos dados do usuário
      if (refreshUserData) {
        await refreshUserData();
        console.log('Dados do usuário atualizados via context');
      }
      ...
      setAvatarFile(null); // Limpar o avatar em caso de erro
            ...
            <div className="relative">
              <Avatar className="h-28 w-28 relative overflow-hidden">
                {avatarFile ? (
                  ...
                  // Foto do Firebase Auth com indicador de carregamento
                  <>
                    <AvatarImage 
                      src={`${user.photoURL}?timestamp=${Date.now()}`} // Forçar refresh
                      alt={user.displayName || ''}
                      className="object-cover w-full h-full"
                      onError={(e) => {
