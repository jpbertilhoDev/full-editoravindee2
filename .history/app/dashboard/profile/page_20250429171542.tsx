...
// Componente específico para exibir avatar com tratamento de erro
const ProfileAvatar = ({ 
  src, 
  userName, 
  onError,
  ...
  const [urlWithCache, setUrlWithCache] = useState<string>('');
  
  useEffect(() => {
    if (src) {
      const newUrl = `${src}?t=${Date.now()}`;
      console.log("Definindo nova URL com cache-busting:", newUrl);
      setUrlWithCache(newUrl);
    }
  }, [src]);
  ...
  return (
    <img 
      ref={avatarRef}
      src={urlWithCache || src}
      alt={userName || 'Avatar'}
      className="h-full w-full object-cover"
      loading="eager"
      crossOrigin="anonymous"
      onLoad={() => {
        console.log("Avatar carregado com sucesso:", urlWithCache || src);
        onLoad();
      }}
      onError={() => {
        console.error("Erro ao carregar avatar:", urlWithCache || src);
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    if (user?.photoURL) {
      console.log("photoURL atualizado:", user.photoURL);
      setAvatarError(false);
      // Verificar se a URL é válida tentando carregar a imagem programaticamente
      const img = new Image();
      img.onload = () => {
        console.log("Imagem verificada e válida:", user.photoURL);
        setAvatarError(false);
      };
      img.onerror = () => {
        console.error("Erro ao verificar imagem programaticamente:", user.photoURL);
        setAvatarError(true);
      };
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Referência para o storage do Firebase com nome de arquivo único
      ...
      console.log("URL de download original:", downloadURL);
      
      // Remover possíveis parâmetros anteriores para evitar problemas
      let cleanURL = downloadURL;
      if (cleanURL.includes('?')) {
        cleanURL = cleanURL.split('?')[0];
      }
      
      // Adicionar timestamp como parâmetro para evitar cache
      const cacheBustURL = `${cleanURL}`;
      console.log("URL final que será usada:", cacheBustURL);
      ...
      try {
        ...
        // Esperar um momento para que os dados possam ser atualizados no contexto
        setTimeout(() => {
          console.log("Verificando usuário após refresh:", auth.currentUser?.photoURL);
          // Atualizar o estado novamente com os dados mais recentes
          if (auth.currentUser) {
            setUser({
              ...auth.currentUser
            });
            
            // Forçar um reset do erro de avatar
            setAvatarError(false);
          }
        }, 1000);
      } catch (refreshError) {
      ...
      // Limpar possíveis imagens antigas em cache
      if ('caches' in window) {
        try {
          // Tentar limpar cache para a URL específica
          caches.open('image-cache').then(cache => {
            if (cleanURL) {
              cache.delete(cleanURL).then(deleted => {
                console.log("Cache limpo:", deleted);
              });
            }
          });
        } catch (cacheError) {
      ...
      const forceReload = () => {
        if (auth.currentUser?.photoURL) {
          ...
          tempImg.src = auth.currentUser.photoURL + '?nocache=' + new Date().getTime();
          console.log("Forçando recarga da imagem:", tempImg.src);
        }
      };
  ...
  const handleRemoveAvatar = async () => {
    ...
    try {
      ...
      // Tentar remover todas as imagens na pasta do usuário
      if (user.photoURL) {
        try {
          // Extrair o caminho da imagem a partir da URL
          console.log("Tentando excluir foto atual:", user.photoURL);
          
          // Se temos uma referência direta na URL, podemos tentar remover
          // Aqui presumimos que a URL será algo como .../avatars/user.uid/filename
          
          // Primeira abordagem: tentar usar a pasta do usuário diretamente
          const userAvatarRef = ref(storage, `avatars/${user.uid}`);
          
          console.log("Removendo avatar do usuário...");
          
          // Tentar excluir arquivo específico se temos a URL
          try {
            await deleteObject(ref(storage, user.photoURL));
            console.log("Arquivo específico removido com sucesso");
          } catch (specificError) {
            console.warn("Erro ao remover arquivo específico:", specificError);
            
            // Se falhou, tentamos remover a pasta inteira
            try {
              const listResult = await listAll(userAvatarRef);
              console.log(`Encontrados ${listResult.items.length} arquivos para remover`);
              
              // Excluir todos os arquivos da pasta
              const deletePromises = listResult.items.map(item => {
                console.log("Excluindo:", item.fullPath);
              ...
              console.warn("Erro ao listar/remover arquivos da pasta:", folderError);
            }
          }
        } catch (storageError) {
          console.warn("Aviso: Não foi possível excluir arquivos do storage", storageError);
        }
      }
            ...
            <div className="relative">
              <div className="h-28 w-28 rounded-full overflow-hidden relative">
                ...
                {avatarFile ? (
                  ...
                  // Foto do Firebase com componente específico para tratamento
                  <ProfileAvatar
                    src={user.photoURL}
