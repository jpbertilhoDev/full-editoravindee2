...
export default function ProfilePage() {
  ...
  const { user, refreshUserData, setUserData } = useAuth();
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Usar URL local temporária para acelerar a exibição
      const localAvatarUrl = `/avatars/${filename}`;
      ...
      // Salvar URL no Firestore primeiro (usando URL local)
      const userDocRef = doc(db, "users", user.uid);
      await updateDoc(userDocRef, {
        photoURL: localAvatarUrl,
        avatarUpdatedAt: serverTimestamp()
      });
      console.log("URL local salva no Firestore");
      
      // Atualizar o Auth com URL local
      await updateProfile(auth.currentUser!, {
        photoURL: localAvatarUrl
      });
      console.log("Auth profile atualizado com URL local");
      ...
      setUserData(prev => ({ ...prev, avatarUrl: localAvatarUrl }));
    } catch (error) {
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Compressão simples da imagem
      const compressedImage = await compressImage(file);
      
      // Cria FormData para enviar ao endpoint
      const formData = new FormData();
      formData.append('file', compressedImage);
      
      // Envia para o endpoint de API
      ...
      toast({
        ...
        description: "Sua imagem foi atualizada e já está sendo exibida.",
      });
      ...
      toast({
        ...
        description: "Não foi possível atualizar sua imagem. Tente novamente mais tarde.",
      });
    } finally {
      setIsUploading(false);
    }
            ...
            <div className="w-full mt-6 pt-4 border-t">
              <div className="space-y-3">
                ...
                <div className="flex items-center text-sm">
                  ...
                  <span>{addressForm.watch('city') ? `${addressForm.watch('street')}, ${addressForm.watch('number')}${addressForm.watch('complement') ? `, ${addressForm.watch('complement')}` : ''}, ${addressForm.watch('neighborhood')}, ${addressForm.watch('city')}, ${addressForm.watch('country')}` : '-'}</span>
                </div>
              </div>
            </div>
          </CardContent>
