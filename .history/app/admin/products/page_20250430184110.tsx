...
// Cache para dados de produtos
const productsCache = new Map<string, {data: Product[], timestamp: number, totalCount: number}>();
const CACHE_TTL = 60 * 1000; // 1 minuto em millisegundos
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      // Criar uma chave de cache com base nos filtros atuais
      const cacheKey = `products_${statusFilter}_${categoryFilter}_${sortField}_${sortDirection}_${currentPage}_${searchQuery}`;
    ...
    fetchProducts();
  }, [toast, statusFilter, categoryFilter, sortField, sortDirection, currentPage, lastVisible]);

  // Adicione um useEffect separado para pesquisa com debounce
  useEffect(() => {
    const timer = setTimeout(() => {
      if (searchQuery) {
        // Aplica o filtro localmente se já tivermos produtos carregados
        const query = searchQuery.toLowerCase();
                            ...
                            {product.images && product.images.length > 0 ? (
                              <img 
                                ...
                                onError={(e) => {
                                  ...
                                  console.log(`Imagem não carregada: ${product.images[0]}`);
                                }}
                              />
                            ) : (
