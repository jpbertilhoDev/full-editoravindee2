...
// Create a memory cache for admin status and data
const adminStatusCache = new MemoryCache();
...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  // Verificar se o usuário é administrador - otimizado com useCallback e caching
  const checkAdminStatus = useCallback(async () => {
    ...
    const cacheKey = `admin_status_${user.uid}`;
    ...
    try {
      ...
      // Cache the admin status for 15 minutes
      adminStatusCache.set(cacheKey, isUserAdmin, 900);
  ...
  // Memorize navigation items to prevent unnecessary rerenders
  ...
  // Memoize the active nav item to prevent recalculation
          ...
          <div className="flex flex-col justify-between flex-1 overflow-y-auto">
            <div className="px-3 py-4 space-y-1">
              {navItems.map((item, index) => (
                <Link href={item.href} key={index} prefetch={true} shallow={true}>
                  <Button 
            ...
            <div className="p-3 border-t">
              <div className="flex items-center gap-3 mb-3 p-2">
                <Avatar className="h-9 w-9 bg-[#08a4a7]">
                  {user?.photoURL ? (
                    <AvatarImage src={user.photoURL} alt={user.displayName || ''} loading="lazy" />
                  ) : (
