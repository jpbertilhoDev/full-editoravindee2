...
export default function BlogPage() {
  ...
  const fetchBlogPosts = useCallback(async (pageNumber = 1, category = 'todas') => {
    console.log(`Buscando posts: página ${pageNumber}, categoria: ${category}`);
    if (pageNumber === 1) {
      setIsLoading(true);
    } else {
    ...
    try {
      ...
      console.log(`Recebidos ${data.posts?.length || 0} posts da API`);
      ...
      if (pageNumber === 1) {
        ...
        setCategories(uniqueCategories.filter(c => c).sort());
      ...
      if (pageNumber === 1) {
        // Tentar criar alguns posts de teste para exibir algo
        const testPosts: BlogPost[] = [
          {
            id: '1',
            title: 'Bem-vindo ao nosso blog',
            excerpt: 'Este é um post para demonstrar que o blog está funcionando.',
            content: '',
  ...
  useEffect(() => {
    ...
    fetchBlogPosts(1, 'todas')
      .then(() => console.log("Posts carregados com sucesso"))
