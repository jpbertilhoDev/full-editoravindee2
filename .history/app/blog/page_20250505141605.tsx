...
export default function BlogPage() {
  ...
  const fetchBlogPosts = useCallback(async (pageNumber = 1, category = 'todas') => {
    ...
    // Debugging: log cache status
    console.log('Cache status:', { 
      hasCachedData: !!cachedData, 
      cacheKey, 
      cacheTTL: CACHE_TTL,
      cacheAge: cachedData ? now - cachedData.timestamp : 'N/A'
    });
    
    // Return cached data if available and fresh
    if (cachedData && now - cachedData.timestamp < CACHE_TTL) {
      console.log('Using cached data', { postsCount: cachedData.posts.length });
      if (pageNumber === 1) {
    ...
    try {
      ...
      if (pageNumber === 1) {
        ...
        if (pageNumber === 1) {
          ...
          if (catResponse.ok) {
            ...
            console.log('Using categories from posts as fallback');
      ...
      console.log('Fetch complete, setting loading states to false');
      setIsLoading(false);
      setIsLoadingMore(false);
    }
  }, [cache]);

  // Alternative function using test API
  const fetchTestPosts = useCallback(async () => {
    setIsLoading(true);
    setError(null);
    
    try {
      console.log('Fetching from test API');
      ...
      if (data.posts && data.posts.length > 0) {
        console.log('Test posts loaded:', data.posts.length);
        ...
        // Set categories to default test category
        setCategories(['Teste', 'Vida Cristã', 'Estudo Bíblico']);
        ...
        toast({
          ...
          description: "Posts de teste carregados com sucesso.",
        });
      } else {
        toast({
          title: "Erro no modo de teste",
          description: "Não foi possível carregar os posts de teste.",
          variant: "destructive",
        });
      }
    } catch (error) {
      console.error("Erro ao buscar posts de teste:", error);
      setError("Não foi possível carregar os posts de teste. Erro de conexão.");
    } finally {
      setIsLoading(false);
    }
  }, []);
      ...
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <div className="flex flex-col justify-center items-center py-10">
          ...
          <div className="flex gap-4 mt-8">
            <Button 
              onClick={() => {
                console.log('Debug reloading posts');
                cache.clear();
                fetchBlogPosts(1, 'todas');
              }}
              className="bg-red-500 hover:bg-red-700 text-white"
            >
              Recarregar API normal
            </Button>
            ...
            >
              Carregar API de teste
            </Button>
          </div>
          
          <div className="mt-6 text-sm text-gray-500">
            <p>Estado atual:</p>
            <pre className="mt-2 text-left bg-gray-100 p-4 rounded">
              {JSON.stringify({
                isLoading,
