...
// Esta função é necessária para gerar rotas estáticas para cada categoria
export async function generateStaticParams() {
  // Include both featured categories and additional slugs used in navigation
  ...
  // Get slugs from featured categories
  const featuredCategorySlugs = featuredCategories.map((category) => ({
    category: category.slug,
  }));
  
  // Combine all slugs, remove duplicates by converting to Set and back to array
  const allParams = [...featuredCategorySlugs, ...additionalSlugs];
  const uniqueSlugs = [];
  const seen = new Set();
  
  for (const param of allParams) {
    if (!seen.has(param.category)) {
      seen.add(param.category);
      uniqueSlugs.push(param);
    }
  }
...
export default function CategoryPage({ params }: CategoryPageProps) {
  ...
  if (categorySlug === 'bestsellers') {
    ...
    filteredProducts = products.filter(product => {
      ...
      // Handle subcategory match
      if (product.subcategory?.toLowerCase() === category.name.toLowerCase()) {
        return true;
      }
