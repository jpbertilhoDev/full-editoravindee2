...
// ----- Configurações -----
const DEEPL_API_KEY = process.env.DEEPL_API_KEY || "33ebc8a0-8a5c-4959-8e51-0c497f729760:fx";
const CACHE_EXPIRATION = parseInt(process.env.TRANSLATION_CACHE_TIME || "3600", 10) * 1000; // milissegundos
const API_REQUEST_DELAY = 100; // ms entre batches de API calls para evitar rate limits
const TEXTS_PER_BATCH = 50; // Quantidade de textos por chamada de API
...
class TranslationCache {
  ...
  setBatchTranslations(lang: string, texts: string[], translations: string[]): void {
...
async function batchTranslate(
  ...
  try {
    ...
    console.log(`Traduzindo ${uniqueTexts.length} textos para ${targetLang}`);
    ...
    response.data.translations.forEach((t: any, i: number) => {
      translationMap.set(uniqueTexts[i], t.text);
    });
 ...
 */
async function processTranslations(source: Record<string, any>, targetLang: string): Promise<Record<string, any>> {
  // Função recursiva para processar todos os textos em um objeto aninhado
  async function processObject(obj: Record<string, any>): Promise<Record<string, any>> {
    ...
    if (textsToTranslate.length > 0) {
      ...
      for (let i = 0; i < textsToTranslate.length; i += TEXTS_PER_BATCH) {
        ...
        const cachedTranslations = translationCache.getBatchTranslations(targetLang, batch);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const sourceData = {
      // Adicionar traduções principais (pode ser expandido)
      footer: {
