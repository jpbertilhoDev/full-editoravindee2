...
export const revalidate = 3600; // Revalidate every hour instead of 10 minutes
...
export async function GET(request: NextRequest) {
  ...
  const cacheKey = `blog-${slug || "all"}-${page}-${postsPerPage}-${category || "none"}-${featured || "none"}`;
  const now = Date.now();
  ...
  if (cachedData && now - cachedData.timestamp < CACHE_TTL) {
    return NextResponse.json(cachedData.data, {
      headers: { "X-Cache": "HIT", "Cache-Control": "public, max-age=60, s-maxage=3600" }
    });
  }
  ...
  try {
    ...
    return NextResponse.json(
      ...
      { status: 200 } // Return 200 with empty posts to prevent cascade failures
    );
  }
}
...
async function getAllPosts(page: number, postsPerPage: number, category: string | null, featured: string | null) {
  ...
  try {
    ...
    if (snapshot.empty) {
      return { posts: [], pagination: { totalCount: 0, currentPage: page, totalPages: 0, hasMore: false } };
    }
